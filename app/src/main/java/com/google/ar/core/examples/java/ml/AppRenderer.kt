...
class AppRenderer(val activity: MainActivity) : DefaultLifecycleObserver, SampleRender.Renderer, CoroutineScope by MainScope() {
  ...
  private val safeDistance = 2.0f // Safe distance in meters
  ...
  override fun onDrawFrame(render: SampleRender) {
    ...
    for (arDetectedObject in arLabeledAnchors) {
      ...
      val distance = calculateDistance(cameraPose, anchorPose)

      // Update UI with distance information
      activity.runOnUiThread {
        try {
          view.distanceTextView?.let { textView ->
            textView.text = String.format("Distance: %.2f meters", distance)
          }

          view.safeDistanceMessage?.let { textView ->
            if (distance < safeDistance) {
              textView.text = "Too close. Move back."
              textView.setTextColor(Color.RED)
            } else {
              textView.text = "Safe distance"
              textView.setTextColor(Color.GREEN)
            }
          }
        } catch (e: Exception) {
        Log.e(TAG, "Error updating distance TextViews", e)
      }
      }

      // Draw label with distance
      val labelWithDistance = "${arDetectedObject.label}\n${String.format("%.2f", distance)}m"
      labelRenderer.draw(
        render,
        viewProjectionMatrix,
        anchor.pose,
  ...
  private fun calculateDistance(cameraPose: Pose, anchorPose: Pose): Float {
...
data class ARLabeledAnchor(val anchor: Anchor, val label: String, var distance: Float = 0f)